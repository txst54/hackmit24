...
def get_free_slot(service, time_slots: List[Tuple[datetime, datetime]], duration: timedelta) -> Tuple[datetime, datetime]:
    for start, end in time_slots:
        ...
        try:
            ...
            for event in events:
                ...
                event_start = event_start.replace(tzinfo=tzlocal()) if event_start.tzinfo is None else event_start
                if current_time + duration <= event_start:
                    return (current_time, current_time + duration)
                event_end = datetime.fromisoformat(event['end'].get('dateTime', event['end'].get('date')))
                event_end = event_end.replace(tzinfo=tzlocal()) if event_end.tzinfo is None else event_end
...
def schedule_meeting(service, email_content: str, sender_email: str, calendar_id: str) -> str:
    time_slots = extract_time_slots(email_content)
    meeting_duration = timedelta(minutes=30)
    
    free_slot = get_free_slot(service, time_slots, meeting_duration)
